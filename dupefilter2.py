...
class DupInfo(object):
	...
	def get_tweet_per_user(self, collection):
		c = Counter()
		tweet_count = 0
		for tweet in collection:
			if tweet_count % 1000 == 0:
	...
	# return user: list of urls dictionary
	def get_url_per_user(self, collection):
		...
		for tweet in collection:
			...
			tweet_count += 1
			if 'user' in tweet:
				c[tweet['user']['id']] += get_embedded_url(tweet)
		return c
	...
	def filter_duplicate_group(self):
		...
		if not os.path.isfile(self.PREFIX + 'duplicate_spam_group.json') or (self.startover):
			...
			# its status here
			if len(dic) -1 not in group_include:
		...
		print "[dupefilter2] loading duplicate_group..."
		id_exclude = set([item for i in duplicate_group.values() for item in i['subgroup']])
		result = []
		
		for index, i in enumerate(range(0, len(dic))):
			if i not in id_exclude:
					entry = dic[i]
                    #name = '_'.join([str(entry['month']), str(entry['day']), str(entry['hour'])]) + "_single_group_" + str(index+1)
					#for reference purpose, id is a better book keeping number
					#name = str(entry['id'])
					
					"""
					
					Important: for backward compatability, use
					name = '_'.join([str(entry['month']), str(entry['day']), str(entry['hour'])]) + "_" + self.clean_filename(entry['text'][:40]) + '_'
					on 9 old datasets
					when crawling user groups
		...
		for name, entry in duplicate_group.iteritems():
			result.append({'userlist': entry['user_ids'], 'url': entry['url'],
							   'filename': name})
