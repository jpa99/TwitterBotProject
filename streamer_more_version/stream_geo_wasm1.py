...
# name: stream_2017_geo2 (used on CLEAR)
consumer_key="0smhqesXvAHTm8zx4psTKTySl"
consumer_secret="riDfBaOxefjg4HL9Y23kxlWv7agMiUbUvGLGXgVB9CVKMbH17x"
access_token="1979279791-SeGtgHizXLiIfNBMKqqMbCOVrO8edIo4cl0tLfj"
access_token_secret="sCOrlxAWGEWjoNQI1HHFHQPxAnRPScfF8zXSSbLW2Nw2c"
			...
			if "text" in data_string:
				...
				if self.save_file:
					...
					if data_string['user']['id'] not in self.unique_user:
						...
						data = data_string['user']
						data["embedded_url"] = data_string['entities']['urls'][0]['expanded_url']
...
def collect(keyword = None, filename = None, num_tweets = 10, save_file = True, print_info = True, save_common = False):
	...
	stream.filter(track=kw, languages=['ar'])       
...
def check_user_location():
	...
	with open(filename, 'r') as f:
		for line in f:
			#print line
			t = json.loads(line)
			url_expand = crawler.unshorten_url(t['embedded_url'], 1, {})
			print url_expand
			try:
				if 'wasm1' in url_expand[2]:
					good_user.append((t['id'], t['screen_name']))
			except Exception, e:
				print e
	print good_user
...
if __name__ == '__main__':
	...
	#collect(keyword = keyword, filename = filename, num_tweets = 10000, save_file = True, print_info = True, save_common = False)
	check_user_location()
