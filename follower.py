...
class FollowerFinder(object):
    ...
    def __init__(self, prefix, userlist):
        self.prefix = prefix
    ...
    def getFollowers(self, user_ids, filename = None):
        ...
        for user in user_ids:
            ...
            try:
                ...
                related_followers = [f for f in followers if f in self.userlist]
        ...
        print "writing raw follower data to json file"
    ...
    def hasfile(self, filename = None):
        if not filename:
                filename = self.prefix + "user_followers_dic.p"
        else:
            filename += "user_followers_dic.p"
    ...
    def load_file(self, filename = None):
        ...
        # else:
        #     filename += "_user_followers_dic.json"
        jsonfields = json.load(open(filename, "r"))
        return jsonfields
...
def get_social_network(prefix, group_id):
    ...
    exit()
    g = social_network.Graph(myFollowerFinder.load_file(filename = filename), TYPE)
    g.build_graph()
...
if __name__ == '__main__':
    #
    # for dld group, 271935813 in group[g]['spam_user'] music dj spammers, group id 4 (fully connected)
    #
    # for goo320 group, id 15, len==42,linkshrink.net, spam group but (all singletons)
    # linkshrink.net goo320, group id 15
    #
    # bit, bot followers, test if they follow each other id 8 don't have a connected graph, id 15 has

    """ IMPORTANT !!! """
    #
    #prefix = 'dld'
    #group_id = 1
    #get_social_network(prefix, group_id)
    prefix = 'tinyurl24'
    for group_id in range(1,56):
        get_social_network(prefix, group_id)
    prefix = 'fb'
